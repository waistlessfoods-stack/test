  ...
  return (
    <div className="w-full min-h-screen bg-white overflow-x-hidden font-metropolis">
      ...
      <section className="bg-[#F4F4F4] px-4 md:px-12 py-20 flex flex-col items-center">
        <h2 className="font-bebas text-[60px] md:text-[80px] text-black mb-10 leading-none">
          RECIPE GALLERY
        </h2>

        {/* SEARCH BAR - Besar & Dominan */}
        <div className="relative w-full mb-16">
          <Input
            placeholder="Find something food"
            className="w-full h-[100px] bg-white rounded-none px-10 border-none shadow-sm focus-visible:ring-0 
              text-[26px] font-metropolis text-black 
              placeholder:font-metropolis 
              placeholder:font-normal 
              placeholder:text-[22px] 
              placeholder:leading-[25.61px] 
              placeholder:tracking-[-0.02em] 
              placeholder:text-black/50"
          />
          <Search className="absolute right-10 top-1/2 -translate-y-1/2 w-10 h-10 text-black stroke-[2.5]" />
        </div>
              ...
              {categories.map((cat, i) => (
                ...
                >
                  <div className="relative h-[200px] rounded-3xl overflow-hidden group cursor-pointer shadow-md">
                    ...
                    <h3 className="relative z-10 h-full flex items-center justify-center font-bebas text-[48px] text-white uppercase">
                      {cat.name}
                    </h3>
                  </div>
                </CarouselItem>
          ...
          {recipes.map((item, idx) => (
            <div key={idx} className="group cursor-pointer">
              <div className="relative aspect-square w-full rounded-[48px] overflow-hidden bg-white shadow-xl mb-10">
                ...
                <div
                  className="absolute top-0 right-0 bg-[#0F8DAB] w-[220px] h-[220px] flex justify-end items-start"
                  style={{ clipPath: "polygon(0 0, 100% 0, 100% 100%)" }}
                >
                  <span className="font-metropolis font-bold text-[48px] text-white pt-10 pr-5">
                    {item.price}
                  </span>
                </div>
                ...
                {item.price !== "Free" && (
                  <div className="absolute bottom-0 left-0 right-0 flex justify-center">
                    <Button className="w-[90%] h-[85px] bg-[#0F8DAB] hover:bg-[#0d7a94] text-[32px] font-bold tracking-[4px] rounded-t-[35px] rounded-b-none border-none">
                      FEATURED
                    </Button>
                  </div>
                )}
              ...
              <div className="flex flex-col gap-4 px-2">
                <h4 className="font-metropolis font-bold text-[42px] text-black leading-tight group-hover:text-[#0F8DAB] transition-colors">
                  {item.title}
                </h4>
                <p className="font-metropolis text-[24px] leading-[1.4] text-gray-800 line-clamp-2">
                  {item.desc}
                </p>
              </div>
            </div>
